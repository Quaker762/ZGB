...
uint8_t Z80::tick()
{
    ...
    case 0x18: //Add N to current address and jump to it
        {
            PC.Word += (signed)mmu->memRead(PC.Word + 1); //The immediate 8 bit value is signed
    ...
    case 0x20:
        {
            if(!isFlagSet(ZERO))
            {
                PC.Word += (signed)mmu->memRead(PC.Word + 1);
                return 12;
                break;
            }
